...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <form action="{{ url("$url/$gameProduk->id") }}" method="post" enctype="multipart/form-data">
                @method('put')
                @csrf
                <input type="hidden" name="idGMaster" value="{{ $gameProduk->idGMaster }}">
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="form-group">
                                <label>Nama Game</label>
                                <input type="text" id="idGMaster" class="form-control" name="nama" value="{{ $gameProduk->game->nama }}" disabled required>
                            </div>
                            {{-- <div class="form-group">
                                <label>Pilih Game</label>
                                <select name="idGMaster" class="form-control">
                                    @foreach ($game as $g)
                                        @if ($g->id == $gameProduk->id)
                                            <option value="{{ $g->id }}" disabled selected>{{ $g->nama }}</option>
                                        @endif
                                    @endforeach
                                </select>  
                            </div> --}}
                            <div class="form-group">
                                ...
                                <input type="text" id="nama" class="form-control" name="nama" value="{{ $gameProduk->nama }}" required>
                            </div>
                            <div class="form-group">
                                <label>Harga Produk Game</label>
                                <input type="number" id="harga" class="form-control" name="harga" value="{{ $gameProduk->harga }}" required>
                            </div>
                            <div class="form-group">
